...
#define p 9 // O número de cruzamentos por geração é igual a 9
#define c 3 // Cada disputa do torneio possui 3 concorrentes

struct Cromossomo // Struct que define o cromossomo
{
    int v[5]; // Cada cromossomo possui um vetor de inteiros indicando o caminho (1->2->4->3->5) = [1,2,4,3,5]
    float fitness; // Cada cromossomo possui uma avaliação (fitness)
};

void inicializa_vetores(int v[]); // Inicializa o vetor de inteiros dos cromossomos
int calcula_fitness(int v[]); // Calcula o fitness de cada cromossomo com base no peso entre as cidades
int peso_entre(int a, int b); // Calcula o peso entre as cidades
int seleciona_pai_torneio(Cromossomo pop[]); // Faz a seleção dos pares pra cruzamento utilizando a técnica de torneio
void cruzamento(Cromossomo c1, Cromossomo c2, Cromossomo f[]); // Faz o cruzamento ponto-um de dois indivíduos e gera 2 filhos. Ex: [1,2,3|5,4] e [3,1,2|4,5] -> [1,2,3,4,5] e [3,1,2,5,4]
void validacao(int v[]); // Faz a validação de um cromossomo para impedir caminhos repetidos. Ex: [1,2,2,3,5] -> [1,2,4,3,5]
void descobre_melhores(Cromossomo pop[], int melhores[]); // Descobre quais os 2 indivíduos que tem o melhor fitness para depois fazer o elitismo com os mesmos
void mutacao(int v[]); // Realiza a mutação de um indivíduo simplesmente trocando duas cidades de posição no mesmo. Ex: [1,2,4,5,3] -> [1,2,3,5,4]
void imprime(Cromossomo pop[]); // Imprime todos os cromossomos. Mostra tanto o vetor de inteiros (o caminho) quanto o fitness de cada um.
...
int main ()
{
    ...
    imprime(pop);
    ger++;
    while(solucao==false) // Critério de parada adotado: Tempo de 0.6 segundos
    {
